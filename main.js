...
지금 바로 지문 본문만 출력하세요. 제목·설명·번호 없이 본문 텍스트만.`;
            ...
            // 2차 API: 문제·선택지·정답 생성 (해설 제외)
            new obsidian.Notice('❓ [2/3] 문제·선택지 생성 중...', 5000);
            const questionPrompt =
`당신은 LEET 언어이해 문제 출제 전문가입니다.
아래 지문을 읽고 문제 3개와 선택지, 정답만 완성하세요.
해설은 작성하지 마세요. 지문은 다시 출력하지 마세요.
...
• 3번: 지문 개념 2개 이상 동시 적용 필요`;
            ...
            // 3차 API: 해설 생성
            new obsidian.Notice('📝 [3/3] 해설 생성 중...', 5000);
            const explanationPrompt =
`당신은 LEET 언어이해 해설 전문가입니다.
아래 지문과 문제를 읽고 해설만 작성하세요.
지문과 문제는 다시 출력하지 마세요. 해설만 출력하세요.
...
1번: [정답 근거 + 각 오답 함정 유형 — 부분일치/주체혼동/인과혼동/조건누락/과잉일반화 분류]

2번: [추론 연쇄 단계 설명 — 각 선택지의 추론 경로와 오류 설명]

3번: [보기 조건과 지문 개념 대응 설명 — 지문 개념 2개 이상 동시 적용 근거]`;

            let explanation = '';
            try {
                explanation = await this.callAPI(explanationPrompt);
                if (!explanation) explanation = '> ⚠️ 해설 생성 실패.';
            } catch (e) {
                explanation = `> ⚠️ 해설 생성 오류: ${e.message}`;
            }
